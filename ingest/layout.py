...
宝宝树 2020 年报（H 股）的财务报表**没有框线**，于是它只抽出数字列：
...
#:
#: 右对齐的大数字在 PDF 里会被从中间断开（实测：宝宝树年报里
...
#: 所以那条正则抓不到（实测：宝宝树年报 `非控制性權益 (55 5)`）。
    ...
    **折行标签会被拼回完整。** 不带值的行先攒着；遇到带值的行时，
    ...
    for raw in text.splitlines():
        ...
        if not label and pending:
            label = pending.pop()
        if not label:
            # **标签根本不存在**（实测：宝宝树年报里「Total current assets」
