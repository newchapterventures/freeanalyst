...
#: ⚠️ **合并 / 母公司表的区分还没做。**
#:
#: 招股说明书和年报里两套表都有，内容几乎一样。实测某教育公司招股书：
#: 第 205 页是合并利润表，第 212 页标题只写「利润表」—— 是母公司表，
#: 但打分一样高，结果选到了母公司表（**口径错了，任何勾稽都抓不住**）。
#:
#: 试过用「归属于母公司 / 少数股东权益」给合并表加分，**净负**：
#: 管理层讨论与分析的概览表里也有「归属于母公司股东的净利润」，
#: 一样吃到加分，把某铜业公司的利润表从第 93 页拉到第 15 页。
#:
#: 只保留「母公司 XX 表」的惩罚也不够 —— 母公司表的标题往往就写「利润表」。
#: **这条判据需要更可靠的信号，暂时不做。** 宁可承认判不了，
#: 也不要留一个「有时对有时错」的规则。
